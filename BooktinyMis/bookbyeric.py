    ...
    def input_books_users(self):
        ...
        while book != "none":
            ...
            if len(bk) != 6:
                ...
                #table1.append([bk])
                deltaBook = pd.Series(bk, index=["书号", "书名", "出版社", "作者", "价格", "库存"])
        ...
        while user != "none":
            ...
            users.to_csv("I:\\data_science\\bookmis\\users.csv", index=False)
            print("&&&&&&&&&&&&&&&&&&&  会员表  &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&")
            #print(users)

        books = pd.DataFrame(books)
        users = pd.DataFrame(users)
        books.to_csv(Config.books_url, header=Config.books_header, index=False)
        users.to_csv(Config.users_url, header=Config.users_header, index=False)
        print(users)
    ...
    def borrow(self, student_id, book_id):
        ...
        for i in range(len(self.student_table["会员号"])):
            if int(self.student_table["会员号"][i])==int(student_id):
                print(self.student_table["会员号"][i]),int(student_id)

                #print(f'查询到该会员：{self.student_table.get(["姓名"][i],"不是会员")} ')
                # get(user,tips) 输出整个会员表！

                print(f'查询到{self.student_table["会员号"][i]}号会员是：\n'
                      f'{self.student_table["姓名"][i]} ')
                ...
                for j in range(len(self.book_table["书号"])):
                    if int(self.book_table["书号"][j]) == int(book_id) and int(self.book_table["库存"][j])>0:
                        ...
                        # 减去库存
                        print(f'{self.student_table["姓名"][i]}借走{self.book_table["书名"][j]}')
                        print(f'库存剩余 {self.book_table["书名"][j]}共 {self.book_table["库存"][j]} 本！')
