...
# Keynote
# 解决pickle传递数据，完成计算，退出时存入csv格式文档待完成
# 解决边界问题，如输入不规范的处理
...
class Path:
    # 3 张表的csv存储地址
    # 定义表-1 books_url 存放书的信息
    # 定义表-2 user_url 存放会员信息
    # 定义表-3 log_url 存放日常操作记录
    books_url = "I:\\data_science\\bookmis\\books.csv"
    #books_header = ["书号", "书名", "出版社", "作者", "价格", "库存"]
    books_header = ['bookid','booth','status','bname','publish','aurtor','price','number']
    ...
    bookdata = pd.read_csv(books_url, low_memory=False, encoding="gbk")  # 防止弹出警告
    ...
    # 流水日记账目 定义表-3
    # log_url = "I:\\data_science\\bookmis\\log.csv"
    logcsv_pd = pd.read_csv(log_url, low_memory=False, encoding="gbk")  # 防止弹出警告
    logpd_df = pd.DataFrame(logcsv_pd)
